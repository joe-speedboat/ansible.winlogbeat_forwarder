...
- **Winlogbeat event log selection (Windows):** add curated event-log groups (`baseline`, `powershell`, `task_scheduler`, `applocker`, `rds`, `user_profile`, `fslogix`, `citrix`, `code_integrity`, `laps`, `storage`) with deployment-time channel reporting. Rendered entries default to `ignore_missing_channel: true` so missing optional/vendor channels do not break Winlogbeat.
